...
llegeix el readme.md que es troba a l'arrel per entendre el projecte.

docker-compose.dev.yml
Què fa: Overrideja el command del backend per usar python run.py (Flask dev server amb debug=True) en lloc de gunicorn. I canvia el frontend per usar un Dockerfile diferent (Dockerfile.dev) que en comptes de nginx arrenca el Vite dev server amb HMR.

frontend/Dockerfile.dev
Què fa: Imatge lleugera de Node que executa npm run dev (Vite) en lloc de compilar i servir amb nginx. El codi font es proporciona via el volumes del docker-compose.dev.yml, i - /app/node_modules assegura que s'usin les dependències de dins el contenidor, no les del teu macOS.

make dev
Què fa: make dev carregarà tant el docker-compose.yml base com el docker-compose.dev.yml d'overrides. En canvi, make up (mode producció) seguirà usant només el fitxer base (sense hot reload).

Resum del comportament final

Comando --- Backend --- Frontend
make up --- gunicorn --- nginx (build) --> NO Hot reload
